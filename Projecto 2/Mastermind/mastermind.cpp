...
bool bot(string word, vector<Tema> dictionary, int index, int player) {
	...
	while(play < 11 && matchingLetters != word.size()) {//cada iteraçao corresponde a uma jogada
		...
		if(word.size() == tryWord.size()){//se o tamanho for diferente nao aceita
...
int main()
{
	...
	while (!(playerNumber = atoi(player.c_str()))) {
	...
	while (!(mode = atoi(gameMode.c_str()))) {
	...
	if (playerNumber == 1)
		bot(palavra, dictionary, themeIndex, 1);
	else
		bot(palavra, dictionary, themeIndex, 2);
